...
class Bce {
    ...
    resize() {
        ...
        this.content.style.minWidth = Math.max(maxWidth + 40, this.content.clientWidth) + 'px';
    }
    ...
    highlight(text) {
        ...
        safe = safe.replace(
            ...
            (m, open, name, attrs, close) => {
                ...
                return '<span class="bce-bracket">' + open + '</span>' +
    ...
    setCursor(cursor) {
        ...
        const setPoint = (lineIndex, offset) => {
            ...
            if (lineEl.childNodes.length === 0 ||
                (lineEl.childNodes.length === 1 && lineEl.childNodes[0].nodeName === 'BR')) {
        ...
        const end = (cursor.endLine === cursor.startLine && cursor.endOffset === cursor.startOffset)
            ? start
    ...
    getSelectedText(cursor) {
        if (cursor.startLine === cursor.endLine && cursor.startOffset === cursor.endOffset) {
            return '';
        }
        if (cursor.startLine === cursor.endLine) {
            return this.lines[cursor.startLine].text.substring(cursor.startOffset, cursor.endOffset);
        }
        let result = this.lines[cursor.startLine].text.substring(cursor.startOffset);
    ...
    deleteSelection(cursor) {
        if (!cursor) return;
        if (cursor.startLine === cursor.endLine && cursor.startOffset === cursor.endOffset) return;

        if (cursor.startLine === cursor.endLine) {
            const line = this.lines[cursor.startLine];
            line.text = line.text.substring(0, cursor.startOffset) + line.text.substring(cursor.endOffset);
        } else {
            const first = this.lines[cursor.startLine];
            const last  = this.lines[cursor.endLine];
            first.text = first.text.substring(0, cursor.startOffset) + last.text.substring(cursor.endOffset);
            this.lines.splice(cursor.startLine + 1, cursor.endLine - cursor.startLine);
        }
        this.render();
        this.setCursor({
            startLine: cursor.startLine, startOffset: cursor.startOffset,
            endLine: cursor.startLine, endOffset: cursor.startOffset,
        });
    }
    ...
    insertText(text) {
        ...
        if (parts.length === 1) {
            ...
            this.render();
            this.setCursor({
                startLine: c.startLine, startOffset: before.length + parts[0].length,
                endLine: c.startLine,   endOffset:   before.length + parts[0].length,
            });
        } else {
            ...
            const lastLine = { id: this.newId(), text: parts[parts.length - 1] + after };
            ...
            this.setCursor({
                startLine: finalLine, startOffset: finalOffset,
                endLine: finalLine,   endOffset:   finalOffset,
            });
        }
        this.pushHistory();
    }
    ...
    doAction(action) {
        switch (action) {
            case 'copy':          return this.actionCopy();
            case 'paste':         return this.actionPaste();
            case 'undo':          return this.undo();
            case 'redo':          return this.redo();
            case 'duplicateDown': return this.actionDuplicate(1);
            case 'duplicateUp':   return this.actionDuplicate(-1);
            case 'moveDown':      return this.actionMove(1);
            case 'moveUp':        return this.actionMove(-1);
        }
    }
    ...
    actionDuplicate(dir) {
        ...
        if (dir > 0) {
            ...
            this.render();
            this.setCursor({
                startLine: idx + 1, startOffset: cursor.startOffset,
                endLine: idx + 1,   endOffset:   cursor.endOffset,
            });
        } else {
            this.lines.splice(idx, 0, copy);
            this.render();
            this.setCursor({
                startLine: idx, startOffset: cursor.startOffset,
                endLine: idx,   endOffset:   cursor.endOffset,
            });
        }
        this.pushHistory();
    }
    ...
    actionMove(dir) {
        ...
        this.render();
        this.setCursor({
            startLine: target, startOffset: cursor.startOffset,
            endLine: target,   endOffset:   cursor.endOffset,
        });
        this.pushHistory();
    }
    ...
    tryEmmet() {
        ...
        if (cursor.startLine !== cursor.endLine || cursor.startOffset !== cursor.endOffset) return false;
        ...
        line.text = line.text.substring(0, startReplace) + clean + line.text.substring(cursor.startOffset);

        this.render();
        const newOffset = startReplace + (cursorPos >= 0 ? cursorPos : clean.length);
        this.setCursor({
            startLine: cursor.startLine, startOffset: newOffset,
            endLine: cursor.startLine,   endOffset:   newOffset,
        });
        this.pushHistory();
        return true;
    }
    ...
    matchBinding(e) {
        for (const b of this.keyBindings) {
            const ctrlOk  = b.ctrl  ? (e.ctrlKey || e.metaKey) : !(e.ctrlKey || e.metaKey);
    ...
    handleTab(shift) {
        ...
        const hasSelection = !(cursor.startLine === cursor.endLine && cursor.startOffset === cursor.endOffset);
        ...
        if (hasSelection) {
            ...
            this.setCursor({
                startLine: start, startOffset: Math.max(0, cursor.startOffset + delta),
                endLine: end,     endOffset:   Math.max(0, cursor.endOffset + delta),
            });
            this.pushHistory();
        } else {
            const line = this.lines[cursor.startLine];
            ...
            const target = Math.ceil((currentLen + 1) / this.options.tabSize) * this.options.tabSize;
            const add = ' '.repeat(target - currentLen);

            line.text = line.text.substring(0, cursor.startOffset) + add + line.text.substring(cursor.startOffset);
            this.render();
            this.setCursor({
                startLine: cursor.startLine, startOffset: cursor.startOffset + add.length,
                endLine: cursor.startLine,   endOffset:   cursor.startOffset + add.length,
            });
            this.pushHistory();
        }
    }
    ...
    handleEnter() {
        ...
        this.setCursor({
            startLine: c.startLine + 1, startOffset: newOffset,
            endLine: c.startLine + 1,   endOffset:   newOffset,
        });
        this.pushHistory();
    }
